...
import { generateOrderListExcel } from "../api/_lib/excel.js";
...
async function handler(req, res) {
  ...
  // ── GET /api/orders/export  (admin — formatted Excel download) ──────────
  if (urlPath === "/api/orders/export" && method === "GET") {
    const token = authHeader(req);
    if (!validToken(token)) return json(res, 401, { error: "Not authenticated." });
    const d      = readData();
    const cfg    = readConfig();
    const xlsBuf = await generateOrderListExcel(d.orders, cfg);
    const date   = new Date().toISOString().slice(0, 10);
    res.writeHead(200, {
      "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
      "Content-Disposition": `attachment; filename="tocs-orders-${date}.xlsx"`,
      "Content-Length": xlsBuf.length,
    });
    return res.end(xlsBuf);
  }
