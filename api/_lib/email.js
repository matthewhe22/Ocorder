  ...
  <div style="max-width:620px;margin:0 auto;background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
    ...
    <div style="padding:32px;">
      ...
      <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
        ...
        ${itemRows}
        ${order.selectedShipping?.cost > 0 ? `<tr><td colspan="2" style="padding:8px 12px;color:#666;">Shipping — ${order.selectedShipping.name}</td><td style="padding:8px 12px;text-align:right;">$${(order.selectedShipping.cost).toFixed(2)}</td></tr>` : ""}
...
export function buildCustomerEmailHtml(order, cfg) {
  ...
  const shippingRow = order.selectedShipping?.cost > 0
    ? `<tr><td colspan="2" style="padding:8px 12px;font-size:0.78rem;color:#666;">Shipping — ${order.selectedShipping.name}</td><td style="padding:8px 12px;text-align:right;font-size:0.78rem;color:#666;">$${(order.selectedShipping.cost).toFixed(2)}</td></tr>`
    : "";
